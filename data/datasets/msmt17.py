...
class MSMT17(BaseImageDataset):
    ...
    def __init__(self, root='data', verbose=True, **kwargs):
        ...
        train = self._process_dir(self.train_dir, self.list_train_path)      # 30248
        # val, num_val_pids, num_val_imgs = self._process_dir(self.train_dir, self.list_val_path)
        query = self._process_dir(self.test_dir, self.list_query_path)       # 11659
        gallery = self._process_dir(self.test_dir, self.list_gallery_path)   # 82161
        if verbose:
            print("=> MSMT17 loaded")
            self.print_dataset_statistics(train, query, gallery)

        self.train = train          # 30248
        self.query = query          # 11659
        self.gallery = gallery      # 82161
