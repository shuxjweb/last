...
sys.path.append('.')
from data import make_data_loader_movie as make_data_loader
from engine.trainer import do_train_movie_mgn as do_train
from modeling import build_model
from layers import make_loss_with_mgn
from solver import make_optimizer, WarmupMultiStepLR
from engine.inference import inference_base as inference
import datetime
...
def main(cfg):
    # prepare dataset
    dataset, train_loader, val_loader, test_loader, num_query, num_query_test, num_classes = make_data_loader(cfg)

    # prepare model
    model = build_model(num_classes, 'mgn')        # num_classes=5000
    model = torch.nn.DataParallel(model).cuda() if torch.cuda.is_available() else model

    loss_func = make_loss_with_mgn(cfg, num_classes)  # modified by gu
    ...
    if cfg.train == 1:
        ...
        do_train(cfg, model, train_loader, val_loader, test_loader, optimizer, scheduler, loss_func, num_query, num_query_test, start_epoch, acc_best)
    else:
        # Test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='last')
    parser.add_argument('--data_dir', type=str, default='/data/shuxj/data/PReID/last_new/last/')
    parser.add_argument('--logs_dir', type=str, default=os.path.join(working_dir, 'logs/20201110_movie_mgn/'))
