...
from engine.trainer import do_train_movie_pcb as do_train
from modeling import build_model
from layers import make_loss_with_pcb
...
def main(cfg):
    ...
    # prepare model
    model = build_model(num_classes, 'pcb')        # num_classes=5000
    model = torch.nn.DataParallel(model).cuda() if torch.cuda.is_available() else model

    loss_func = make_loss_with_pcb(cfg, num_classes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--height_mask', type=int, default=384)
    ...
    parser.add_argument('--logs_dir', type=str, default=os.path.join(working_dir, 'logs/20201008_movie_pcb6/'))
