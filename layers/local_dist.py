...
def hard_example_mining(dist_mat, labels, return_inds=False):  # [32, 32], [32,]
    ...
    N = dist_mat.size(0)  # 32

    # shape [N, N]
    is_pos = labels.expand(N, N).eq(labels.expand(N, N).t())  # [32, 32], torch.uint8
    is_neg = labels.expand(N, N).ne(labels.expand(N, N).t())  # [32, 32]

    # `dist_ap` means distance(anchor, positive)
    # both `dist_ap` and `relative_p_inds` with shape [N, 1]
    dist_ap, relative_p_inds = torch.max(
        dist_mat[is_pos].contiguous().view(N, -1), 1, keepdim=True)  # [32, 1], [32, 1]
    # `dist_an` means distance(anchor, negative)
    # both `dist_an` and `relative_n_inds` with shape [N, 1]
    dist_an, relative_n_inds = torch.min(
        dist_mat[is_neg].contiguous().view(N, -1), 1, keepdim=True)  # [32, 1], [32, 1]
    # shape [N]
    dist_ap = dist_ap.squeeze(1)  # [32,]
    dist_an = dist_an.squeeze(1)  # [32,]

    if return_inds:
        # shape [N, N]
        ind = (labels.new().resize_as_(labels)
               .copy_(torch.arange(0, N).long())
               .unsqueeze(0).expand(N, N))  # [32, 32]
        # shape [N, 1]
        p_inds = torch.gather(
        ...
        return dist_ap, dist_an, p_inds, n_inds  # [32,], [32,], [32,], [32,]
...
def batch_local_dist(x, y):
    ...
    # shape [N, m, n]
    dist_mat = batch_euclidean_dist(x, y)  # [32, 8, 8]
    dist_mat = (torch.exp(dist_mat) - 1.) / (torch.exp(dist_mat) + 1.)  # [32, 8, 8]
    # shape [N]
    dist = shortest_dist(dist_mat.permute(1, 2, 0))  # [32,]
    return dist
