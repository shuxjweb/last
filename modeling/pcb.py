...
__all__ = ['pcb_p6', 'pcb_p4', 'pcb_global']
...
class PCB(nn.Module):
    ...
    def forward(self, x):    # [64, 3, 256, 128]
        ...
        v_g = self.parts_avgpool(f)        # [64, 2048, 6, 1]

        if not self.training:
            v_g = F.normalize(v_g, p=2, dim=1)
            return v_g.reshape(v_g.size(0), -1)

        v_g = self.dropout(v_g)            # [64, 2048, 6, 1]
        v_h = self.conv5(v_g)              # [64, 256, 6, 1]

        y = []
        for i in range(self.parts):        # 6
            v_h_i = v_h[:, :, i, :]
            v_h_i = v_h_i.view(v_h_i.size(0), -1)    # [64, 256]
            y_i = self.classifier[i](v_h_i)          # [64, 751]
            y.append(y_i)

        y = torch.stack(y).permute(1, 0, 2)          # [64, 6, 5000]

        v_g = F.normalize(v_g, p=2, dim=1)           # [64, 2048, 6, 1]
        v_g = v_g.view(v_g.size(0), -1)              # [64, 2048*6]

        if not self.training:
            return v_g

        return y, v_g           # [64, 6, 5000], [64, 2048*6]
...
def pcb_p6(num_classes, loss=None, pretrained='imagenet', **kwargs):
    model = PCB(
