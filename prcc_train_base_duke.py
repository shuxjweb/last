...
def main(cfg):
    ...
    if cfg.train == 1:
        start_epoch = 0
        acc_best = 0.0
        last_model_wts = torch.load(os.path.join('pre_feat', 'duke_ini_imagenet.pth'))
        ...
        feats = torch.stack([feat_dict[name] for name, _, _ in val_loader_c.dataset.dataset]).cpu().data.numpy()  # [70264, 2048]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs_dir', type=str, default=os.path.join(working_dir, 'logs/20210204_prcc_base_duke_sgd'))
