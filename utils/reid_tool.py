...
def visualize_ranked_results(distmat, dataset, save_dir='log/ranked_results', topk=20):  # [3368, 15913]
    """
    ...
    """
    num_q, num_g = distmat.shape  # 3368, 15913
    ...
    for q_idx in range(num_q):  # 3368
        ...
        for g_idx in indices[q_idx, :]:
            ...
            invalid = (qpid == gpid) & (qcamid == gcamid)  # True
            if not invalid:
...
def visualize_ranked_results_all(distmat, dataset, save_dir='log/ranked_results', topk=20):  # [3368, 15913]
    """
    Visualize ranked results
    ...
    for q_idx in range(num_q):  # 3368
        ...
        for g_idx in indices[q_idx, :]:
            ...
            invalid = (qpid == gpid) & (qcamid == gcamid)  # True
            if not invalid:
                if rank_idx > topk and int(qpid) != int(gpid):
                    continue
                _cp_img_to(gimg_path, qdir, rank=rank_idx, prefix='gallery')
                rank_idx += 1
